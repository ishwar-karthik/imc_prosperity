...
class Trader:
    ...
    @staticmethod
    def moving_average(data, period, setting="simple"):
        MOVING_AVG_RATE = 0.1  # TODO: FINE-TUNE THIS, maybe move to global
        if len(data) < period:
            return data[-1]  # TODO: think through the logic and define this appropriately
        if setting == "simple":
            return statistics.fmean(data[-period:])
        if setting == "exponential":
            exp_weights = []  # TODO: define the weights
            return statistics.fmean(data[-period:], exp_weights)

    def compute_orders_amethysts(self, state: TradingState, product, order_depth):
    ...
    def compute_orders_starfruit(self, state, product, order_depth):
        ...
        for ask, vol in osell.items():
            if (ask < buying_price) or ((cpos < 0) and (ask == buying_price)):
                if cpos < pos_limit and rsi < oversold_rsi:
                    order_for = min(-vol, pos_limit - cpos)
                    cpos += order_for
                    assert (order_for >= 0)
                    logger.print("BUY", str(Order(product, ask, order_for)))
                    orders.append(Order(product, ask, order_for))
                elif cpos < (pos_limit - momentum_buy):
                    order_for = min(-vol, pos_limit - cpos - momentum_buy)
                    cpos += order_for
                    assert (order_for >= 0)
                    logger.print("BUY", str(Order(product, ask, order_for)))
                    orders.append(Order(product, ask, order_for))
        ...
        cpos = position
        for bid, vol in obuy.items():
            if (bid > selling_price) or ((cpos > 0) and (bid == selling_price)):
                if cpos > -pos_limit and rsi > overbought_rsi:
                    order_for = min(vol, pos_limit + cpos)
                    order_for *= -1  # we want to sell, so negate everything
                    cpos += order_for
                    assert (order_for <= 0)
                    logger.print("SELL", str(Order(product, bid, order_for)))
                    orders.append(Order(product, bid, order_for))
                elif cpos > -(pos_limit - momentum_sell):
                    order_for = min(vol, pos_limit - momentum_sell + cpos)
                    order_for *= -1  # we want to sell, so negate everything
                    cpos += order_for
                    assert (order_for <= 0)
                    logger.print("SELL", str(Order(product, bid, order_for)))
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for product in state.order_depths:
            ...
            if product == "AMETHYSTS":
                result[product] += self.compute_orders_amethysts(state, product, order_depth)
            elif product == "STARFRUIT":
                result[product] += self.compute_orders_starfruit(state, product, order_depth)
            else:
