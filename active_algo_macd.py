...
class Trader:
    ...
    VWAP_STORAGE = 100
    STARTING_VWAP_STORAGE = 300
    MACD_STORAGE = 10

    # Maybe we can simplify these two, since they reduce to VOLATILE and MOMENTUM respectively
    ...
    KEEP_MACD = {product: False for product in PRODUCTS}
    for product in MOMENTUM_PRODUCTS:
        KEEP_MACD[product] = True

    # Percentiles defining overbought and oversold signals (Edward's suggestion)
    PERCENTILE_OVERSOLD = 30
    PERCENTILE_OVERBOUGHT = 70
    ...
    exponential_ma_12 = 0
    exponential_ma_26 = 0
    old_macd = deepcopy(EMPTY_PRODUCT_LIST)
    old_macd_signal = deepcopy(EMPTY_PRODUCT_LIST)
    macd_flag = -1
    ...
    trader_data_dict = {
        ...
        "old_macd": old_macd,
        "old_macd_signal": old_macd_signal,
        "macd_flag": macd_flag,
    }
    ...
    def update_trader_data(self, state, product, data):
        if "vwap" in data.keys():
            ...
            if self.KEEP_STARTING_VWAPS[product]:
                if state.timestamp <= self.STARTING_VWAP_STORAGE * 100:
                    self.starting_vwaps[product].append(vwap)
                    if state.timestamp == self.STARTING_VWAP_STORAGE * 100:
                        # raise Exception(f"reached at timestamp={state.timestamp}")
                        self.percentile_lower = np.percentile(self.starting_vwaps[product], self.PERCENTILE_OVERSOLD)
                        self.percentile_upper = np.percentile(self.starting_vwaps[product], self.PERCENTILE_OVERBOUGHT)

            if self.KEEP_MACD[product]:
                ...
                if state.timestamp == 2600:
                    ...
                    self.exponential_ma_26 = self.exponential_moving_average(self.exponential_ma_26, vwap, 26)

                if state.timestamp >= 2600:
                    macd = self.exponential_ma_12 - self.exponential_ma_26
                    self.old_macd[product].append(macd)
                    if len(self.old_macd) > self.MACD_STORAGE:
                        self.old_macd[product].pop(0)

                if state.timestamp == 3500:  # 9 iterations for MACD signal line
                    simple_ma = statistics.mean(self.old_macd[product])
                    macd_signal = simple_ma
                    self.old_macd_signal[product].append(macd_signal)
                elif state.timestamp > 3500:
                    prev_macd_signal = self.old_macd_signal[product][-1]
                    current_macd = self.old_macd[product][-1]
                    macd_signal = self.exponential_moving_average(prev_macd_signal, current_macd, 9)
                    self.old_macd_signal[product].append(macd_signal)
                    if len(self.old_macd_signal) > self.MACD_STORAGE:
                        self.old_macd_signal[product].pop(0)

                    """
                    # To fetch data for plotting
                    target = "backtests/momentum_macd.csv"
                    with open(target, mode="a") as f:
                        f.write(f"{state.timestamp},{current_macd},{macd_signal}\n")
                        """

        if "position" in data.keys():
            pos = data["position"]
            self.old_positions[product].append(pos)
            if len(self.old_positions[product]) > self.VWAP_STORAGE:
    ...
    def compute_orders_volatile(self, state: TradingState, product, order_depth):
        ...
        past_vwap = sum(self.old_vwaps[product][-self.VWAP_STORAGE:]) / \
            self.VWAP_STORAGE if len(self.old_vwaps) >= self.VWAP_STORAGE else vwap
    ...
    def compute_orders_momentum(self, state: TradingState, product, order_depth):
        ...
        The MACD is calculated by subtracting the 26-period Exponential Moving Average (EMA) from the 12-period EMA. A 9-period EMA of the MACD, called the “signal line,” is then plotted on top of the MACD line, acting as a trigger for buy and sell signals.
        
        When the MACD line crosses above the signal line, it is considered a bullish signal, indicating that the price is likely to rise. Conversely, when the MACD line crosses below the signal line, it is a bearish signal, indicating that the price is likely to fall.
        
        Reference: https://medium.com/@patelsaadn/macd-algorithmic-trading-strategy-simplified-theory-and-python-implementation-d366d39ed351
        """
        pos_limit += 10
        if state.timestamp > 50 * 100:  # start when signal line has had time to develop
            macd = self.old_macd[product][-1]
            macd_signal = self.old_macd_signal[product][-1]
            if macd > macd_signal:
                if self.macd_flag != 1:  # bullish momentum
                    logger.print("OVERSOLD (MACD)")
                ...
                if self.macd_flag != 0:  # bearish momentum
                    logger.print("OVERBOUGHT (MACD)")
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        if state.traderData != '':
            self.trader_data_dict = jsonpickle.decode(state.traderData)
            ...
            self.exponential_ma_12 = self.trader_data_dict["exponential_ma_12"]
            self.exponential_ma_26 = self.trader_data_dict["exponential_ma_26"]
            self.old_macd = self.trader_data_dict["old_macd"]
            self.old_macd_signal = self.trader_data_dict["old_macd_signal"]
            self.macd_flag = self.trader_data_dict["macd_flag"]
        logger.print("traderData: " + state.traderData)
        logger.print("Observations: " + str(state.observations))
        ...
        for product in state.order_depths:
            ...
            if product in self.STABLE_PRODUCTS:
                ...
                pass #result[product] += self.compute_orders_volatile(state, product, order_depth)
            elif product in self.MOMENTUM_PRODUCTS:
                result[product] += self.compute_orders_momentum(state, product, order_depth)
            else:
                raise ValueError(f"{product} is an invalid product!")

        trader_data = jsonpickle.encode(self.trader_data_dict)
